...
$("body").append("<style>\n" +
    "#bakcms-panel {z-index: 13501;position:fixed; top:50px; left:50px; text-align:center; color: black; background: rgba(220,220,220,0.7); border:2px solid rgb(160,160,160); width: 150px; height: 180px; padding: 0.5em; -webkit-transition: background 0.3s; transition: background 0.3s; }\n" +
    "#bakcms-panel:hover, #bakcms-content:hover, #bakcms-blocks:hover, #bakcms-content-add-block:hover {background: rgba(220,220,220,1); }\n" +
    ...
    "#bakcms-editor-menu .btn {margin: 5px auto;}\n" +
    ...
    "#bakcms-content {z-index: 13502;display:none; position:fixed; top:50px; right:50px; color: black; background: rgba(220,220,220,0.7); border:2px solid rgb(160,160,160); width: 600px; height: 360px; padding: 0.5em; -webkit-transition: background 0.3s; transition: background 0.3s;}\n" +
    "#bakcms-content-add-block {z-index: 13503;display:none; position:fixed; bottom:50px; right:50px; color: black; background: rgba(220,220,220,0.7); border:2px solid rgb(160,160,160); width: 220px; height: 120px; padding: 0.5em; -webkit-transition: background 0.3s; transition: background 0.3s;}\n" +
    "#bakcms-blocks {z-index: 13504;display:none; position:fixed; left:50px; bottom:50px; color: black; background: rgba(220,220,220,0.7); border:2px solid rgb(160,160,160); width: 200px; height: 360px; padding: 0.5em; -webkit-transition: background 0.3s; transition: background 0.3s;}\n" +
    "#bakcms-blocks .blocks-list {display: block; height: 300px; overflow-y: scroll; overflow-x: hidden;}\n" +
    "#bakcms-preloader {z-index: 13500;position: fixed;left: 0; top: 0; width:100%; height: 100%; background: rgba(0,0,0,0.7);}" +
"  </style>");



$("body").append("<div data-bakcms-no-parse id=\"bakcms-panel\" class=\"draggable ui-widget-content\">\n" +
...
$("body").append("<div data-bakcms-no-parse id=\"bakcms-content\" class=\"draggable ui-widget-content\">\n" +
...
"<textarea style='width: 100%; height: 280px;' id=\"bakcms-editor\" name='bakcms-editor'></textarea>\n" +
"</div>\n" +
"<div style='display: inline-block;width:20%;float: right;text-align: center' id='bakcms-editor-menu'>\n" +
...
$("body").append("<div data-bakcms-no-parse id=\"bakcms-blocks\" class=\"draggable ui-widget-content\">\n" +
...
$("body").append("<div data-bakcms-no-parse id=\"bakcms-content-add-block\" class=\"draggable ui-widget-content\">\n" +
...
$("body").append("<div id=\"bakcms-preloader\"> </div>");
...
$( window ).on( "load", function() {
    ...
    $("#bakcms-preloader").hide();

    editor = CKEDITOR.replace( 'bakcms-editor', {
...
function shift_up() {
    prev_element.push($(".activated").data("bakcms-id"));
...
function shift_down() {
    ...
    $("[data-bakcms-id='" + prev_id + "']").addClass("activated").addClass("hovered");
    editor.setData($("[data-bakcms-id='" + prev_id + "']").html());
}

$("#bakcms-content .fa-close").click(function () {
    $(".activated").removeClass("activated").removeClass("hovered");
...
$("#bakcms-blocks .fa-close").click(function () {
    $("#bakcms-content-add-block").hide();
    $("#bakcms-blocks").hide();
    $("#bakcms-panel .fa-plus").parent().removeClass("btn-active");
    $(".activated").removeClass("activated").removeClass("hovered");
    $("#bakcms-preloader").hide();
    appender_enabled = false;
});

$("#bakcms-content-add-block .fa-close").click(function () {
    $("#bakcms-content-add-block").hide();
    $("#bakcms-blocks").hide();
    $("#bakcms-panel .fa-plus").parent().removeClass("btn-active");
    $(".activated").removeClass("activated").removeClass("hovered");
...
function appender_mode(el) {
    ...
    selected_block =  $(el).data("bakcms-block-id");
...
function load_blocks() {
    $.ajax({
        ...
        .done(function( msg ) {
            ...
            $("#bakcms-blocks .blocks-list").html("");
            $(data).each(function (i) {
                $("#bakcms-blocks .blocks-list").append("<div onclick='appender_mode(this)' data-bakcms-block-id='" + data[i].element_id + "' class='block-preview'><p>" + data[i].name + "</p><img src='" + data[i].image + "'></div>");
            })
        });

    $("#bakcms-blocks").show();
    $("#bakcms-preloader").show();
...
function handle_activated_element(el) {
    ...
    editor.on( 'change', function( evt ) {
        document.body.addEventListener('DOMNodeInserted', function(e) { if(e.target.nodeType == 1) { e.target.removeAttribute('data-bakcms-no-parse') }});
        $(".activated").html(evt.editor.getData());
        document.body.addEventListener('DOMNodeInserted', function(e) { if(e.target.nodeType == 1) { e.target.setAttribute('data-bakcms-no-parse', '') }});
    });
}

function handle_appended_element(el) {
    $("#bakcms-content-add-block").show();
}
...
function create_block(el) {
    ...
    var elem_id = $(".activated").data("bakcms-id");
    var elem = document.querySelectorAll("[data-bakcms-id='" + elem_id + "']")[0];
    var img = "";
